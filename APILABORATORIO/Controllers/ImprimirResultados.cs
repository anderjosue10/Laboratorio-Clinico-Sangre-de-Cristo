                    ...
                    container.Page(page =>
                    {
                        ...
                        page.Content().Column(column =>
                        {
                            ...
                            var tituloExamen = detalleOrden.IdtipoExamenNavigation.NombreExamen.ToUpper().Contains("CITOLOGÍA FECAL") ||
                                ...
                                ? "PARASITOLOGÍA"
                                : detalleOrden.IdtipoExamenNavigation.NombreExamen.ToUpper().Contains("TOLERANCIA")
                                    ? "QUÍMICA SANGUÍNEA"
                            ...
                            else if (detalleOrden.IdtipoExamenNavigation.NombreExamen.ToUpper().Contains("HEMATOLOGÍA"))
                            {
                                ...
                                // VERIFICAR SI SOLO VIENE EL PARÁMETRO "PLAQUETAS"
                                bool soloPlaquetas = listaResultados.Count == 1 &&
                                                    listaResultados[0].NombreParametro?.ToUpper().Contains("PLAQUETAS") == true;
                                var resultadosPorPagina = new List<List<ResultadoExaman>>();
                                int resultadosPorPaginaCount = 15;
                                ...
                                for (int i = 0; i < resultadosPorPagina.Count; i++)
                                {
                                    ...
                                    if (i == 0)
                                    {
                                        column.Item().Table(headerTable =>
                                        {
                                            ...
                                            // MODIFICACIÓN: No mostrar descripción si solo viene Plaquetas
                                            if (!string.IsNullOrEmpty(detalleOrden.IdtipoExamenNavigation.Descripcion) && !soloPlaquetas)
                                            {
                                    ...
                                    column.Item().Table(resultsTable =>
                                    {
                                        ...
                                        {
                                            // MODIFICACIÓN: No mostrar subtítulo si solo viene Plaquetas
                                            if (!soloPlaquetas)
                                            {
                                                var subtitulo = resultado.IdparametroNavigation?.Subtitulo;
                                                if (subtitulo != currentSubtitulo && !string.IsNullOrEmpty(subtitulo))
                                    ...
                                    column.Item().Table(resultsTable =>
                                    {
                                        ...
                                        foreach (var resultado in resultadosPagina)
                                        {
                                            ...
                                            if (nombreParametro.ToUpper().Contains("CREATININA") &&
                                                !nombreParametro.ToUpper().Contains("ORINA") &&
                                                !nombreParametro.ToUpper().Contains("EN ORINA"))
                                            {
                                                if (edad < 11) // Menores de 10 años (cualquier sexo)
                                                {
                                                    valorReferencia = "0.4 - 0.7 mg/dL";
                                                }
                                                else if (edad >= 11 && edad < 19) // Adolescentes 11-18 años (cualquier sexo)
                                                {
                                                    valorReferencia = "0.6 - 1.1 mg/dL";
                                                }
                                                else if (edad >= 19) // Mayores de 19 años
                                                {
                                                    if (generoPaciente == "F") // Mujeres adultas (19+)
                                                    {
                                                        valorReferencia = "0.6 - 1.1 mg/dL";
                                                    }
                                                    else // Hombres adultos (19+)
                                                    {
                                                        valorReferencia = "0.7 - 1.4 mg/dL";
                                                    }
                                                }
                                            }
                                            else if (nombreParametro.ToUpper().Contains("CREATININA") &&
                                                     (nombreParametro.ToUpper().Contains("ORINA") ||
                                                      nombreParametro.ToUpper().Contains("EN ORINA")))
                                            {
                                                // Mantener el valor de referencia original para Creatinina en Orina
                                                valorReferencia = resultado.IdparametroNavigation?.ValorReferencia;
                                            }
                            ...
                            else if (detalleOrden.IdtipoExamenNavigation.NombreExamen.ToUpper().Contains("TOLERANCIA"))
                            {
                                var listaResultados = resultadosFiltrados.ToList();
                                var resultadosPorPagina = new List<List<ResultadoExaman>>();
                                int resultadosPorPaginaCount = 15;
                                ...
                                for (int j = 0; j < resultadosPorPagina.Count; j++)
                                {
                                    ...
                                    column.Item().Table(resultsTable =>
                                    {
                                        ...
                                        foreach (var resultado in resultadosPagina)
                                        {
                                            ...
                                            // PARA TOLERANCIA: Usar los valores de referencia originales sin lógica de género/edad
                                            // (Se eliminó el bloque completo de condiciones para Química Sanguínea)
                            ...
                            else if (detalleOrden.IdtipoExamenNavigation.NombreExamen.ToUpper().Contains("INMUNOLOGÍA"))
                            {
                                var listaResultados = resultadosFiltrados.ToList();
                                var resultadosPorPagina = new List<List<ResultadoExaman>>();
                                int resultadosPorPaginaCount = 15;

                                // Obtener el género del paciente (para posibles futuras validaciones)
                                var generoPaciente = detalleOrden.IdordenNavigation.IdclienteNavigation.Genero?.ToUpper() ?? "M";
                                var esNino = edad < 18;
                                ...
                                for (int j = 0; j < resultadosPorPagina.Count; j++)
                                {
                                    ...
                                    column.Item().Table(headerTable =>
                                    {
                                        headerTable.ColumnsDefinition(columns =>
                                        {
                                            columns.RelativeColumn(3);     // ANÁLISIS (igual que Química Sanguínea)
                                            columns.RelativeColumn(1.3f);  // RESULTADO (igual que Química Sanguínea)
                                            columns.RelativeColumn(1);     // UNIDAD DE MEDIDA (igual que Química Sanguínea)
                                            columns.RelativeColumn(2.5f);  // VALORES DE REFERENCIA (igual que Química Sanguínea)
                                        });
                                    ...
                                    column.Item().Table(resultsTable =>
                                    {
                                        resultsTable.ColumnsDefinition(columns =>
                                        {
                                            columns.RelativeColumn(3);     // Mismas dimensiones que Química Sanguínea
                                            columns.RelativeColumn(1.3f);  // Mismas dimensiones que Química Sanguínea
                                            columns.RelativeColumn(1);     // Mismas dimensiones que Química Sanguínea
                                            columns.RelativeColumn(2.5f);  // Mismas dimensiones que Química Sanguínea
                                        });
                                        ...
                                        foreach (var resultado in resultadosPagina)
                                        {
                                            ...
                                            // Aplicar lógica de valores por género/edad específica para Inmunología si es necesario
                                            // (Puedes agregar condiciones específicas para parámetros de Inmunología aquí)

                                            resultsTable.Cell().Element(ResultadoStyle).Text(nombreParametro).FontSize(12);

                                            // RESULTADO CON NEGRITA PARA VALORES ANORMALES
                                            resultsTable.Cell().Element(ResultadoStyle).Text(text =>
                                            {
                                                bool esValorAnormal = IsResultOutOfReferenceRange(
                                                    resultado.Resultado ?? "",
                                            ...
                                            // Unidad de medida - Mismo padding que Química Sanguínea
                                            resultsTable.Cell().Element(ResultadoStyle).PaddingLeft(-40).Text(resultado.IdparametroNavigation?.UnidadMedida ?? "").FontSize(12);
                    ...
                    container.Page(page =>
                    {
                        ...
                        page.Content().Column(column =>
                        {
                            ...
                            foreach (var orden in ordenes)
                            {
                                ...
                                {
                                    var nombreExamen = detalle.IdtipoExamenNavigation.NombreExamen.ToUpper();

                                    // Exámenes que siempre se consideran "pequeños" por su formato compacto
                                    var examenesCompactos = new[]
                                    {
                                        "EXÁMENES DIVERSOS"
                                    };

                                    if (examenesCompactos.Any(compacto => nombreExamen.Contains(compacto)))
                                    {
                                        // Para estos exámenes, considerar siempre como pequeños si tienen pocos grupos
                                        var parametrosConResultado = detalle.ResultadoExamen
                                            .Count(r => !string.IsNullOrWhiteSpace(r.Resultado));

                                        if (nombreExamen.Contains("EXÁMENES DIVERSOS"))
                                        {
                                            return parametrosConResultado <= 24; // Máximo 8 líneas (8×3=24 parámetros)
                                        }
                                        else
                                        {
                                            return parametrosConResultado <= 15; // Ajustar según necesidad
                                        }
                                    }
                                    else
                                    {
                                        // Para otros exámenes, usar la lógica normal
                                        return detalle.ResultadoExamen
                                            .Count(r => !string.IsNullOrWhiteSpace(r.Resultado)) <= 11;
                                    }
                                }
                                ...
                                for (int i = 0; i < examenesEnOrden.Count; i++)
                                {
                                    ...
                                    else if (detalleOrden.IdtipoExamenNavigation.NombreExamen.ToUpper().Contains("INMUNOLOGÍA"))
                                    {
                                        ...
                                        for (int j = 0; j < resultadosPorPagina.Count; j++)
                                        {
                                            ...
                                            // Mostrar encabezado si es la primera página
                                            if (j == 0 && !esMismaTabla && (esPrimerExamenEnOrden || !puedenCaberEnUnaPagina || esPrimerExamenEnPagina))
                                            {
                                                column.Item().Table(headerTable =>
                                                {
                            ...
                            foreach (var orden in ordenes)
                            {
                                ...
                                bool EsExamenPequeno(DetalleOrden detalle)
                                {
                                    ...
                                    else
                                    {
                                        // Para otros exámenes, usar la lógica normal
                                        return detalle.ResultadoExamen
                                            .Count(r => !string.IsNullOrWhiteSpace(r.Resultado)) <= 8;
                                    }
                                }
