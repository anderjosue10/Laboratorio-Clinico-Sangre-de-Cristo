...
const onConfirmDate2 = ({ date }) => {
  ...
  const formatDateWithoutTZ = (date) => {
    // Asegúrate de que date es realmente un objeto Date válido
    if (!(date instanceof Date) || isNaN(date)) {
      date = new Date(date); // Intenta convertir si no es Date
    }
    
    const year = date.getFullYear();
    const month = String(date.getMonth() + 1).padStart(2, '0');
    const day = String(date.getDate()).padStart(2, '0');
    return `${year}-${month}-${day}`; // Elimina la parte de tiempo
  };
  ...
  const ordenParaEnviar = {
    ...
    fechaOrden: formatDateWithTime(form.fechaOrden),
    fechaEntrega: formatDateWithTime(form.fechaEntrega),
    estado: 'Pendiente',
  ...
  console.log('Datos a enviar:', ordenParaEnviar); // Verifica en consola las fechas
    try {
